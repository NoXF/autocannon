...
function track(instance, opts) {
  ...
  instance.on('start', () => {
    if (opts.renderProgressBar) {
      ...
      if (!iOpts.amount) {
        logToStream(`Running ${iOpts.duration}s test @ ${opts.displayUrl ? iOpts.url : ''}${socketPath}\n${msg}\n`)

        durationProgressBar = trackDuration(instance, opts, iOpts)
      } else {
        logToStream(`Running ${iOpts.amount} requests test @ ${opts.displayUrl ? iOpts.url : ''}${socketPath}\n${msg}\n`)
